...
export const deletePost = async (req, res) => {
    try {
        ...
        await User.updateOne({ _id: req.userId }, { $pull: { posts: postId, saved: postId } })
...
export const sharePost = async (req, res) => {
    try {
        ...
        // Reuse messaging: send link to post as message text
        const text = message || `Check this post: ${post._id}`

        // Emit a minimal event so client can handle share notification (optional)
        io.emit("sharedPost", { postId, from: req.userId, to: receiverId })

        return res.status(200).json({ message: "post shared", postId, receiverId, text })
    } catch (error) {
